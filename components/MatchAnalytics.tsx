...
export default function MatchAnalytics({ userId }: MatchAnalyticsProps) {
    ...
    return (
        <div className="space-y-8">
            {/* Header / Filter */}
            <div className="bg-white rounded-2xl p-2.5 border-2 border-blue-100 shadow-sm">
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col md:flex-row gap-3">
                        <select
                            ...
                            className="flex-1 px-2.5 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                        ...
                        {graphData.total > 0 && (graphData.firstTurnTotal > 0 || graphData.secondTurnTotal > 0) && (
                            <div className="flex w-full gap-2 mt-3 z-10">
                                ...
                                {graphData.secondTurnTotal > 0 && (
                                    <div className="flex-1 flex justify-between items-center px-3 py-2 bg-blue-50/50 border border-blue-100 rounded-lg">
                                        <span className="text-[10px] font-bold text-blue-600">後攻</span>
                                        <span className="text-sm font-bold text-blue-700">{Math.round(graphData.secondTurnWinRate)}%</span>
                                    </div>
                                )}
                            </div>
                        )}
                                ...
                                {deckPerformanceData.length > 0 ? (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={deckPerformanceData} layout="vertical">
                                            ...
                                            <Bar dataKey="winRate" fill="#2563eb" radius={[0, 4, 4, 0]}>
                    ...
                    {filteredMatches.length > 0 ? (
                        filteredMatches.map((match) => (
                            <div
                                ...
                                className={`bg-white rounded-xl p-3 border shadow-sm hover:shadow-md transition cursor-pointer ${expandedMatchId === match.id ? 'border-blue-300 ring-1 ring-blue-100' : 'border-gray-200'}`}
                            >
                                    ...
                                    <div className="mt-3 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                        <div className="flex justify-between items-center mb-2 px-1">
                                            <div className="text-xs font-bold text-gray-400 flex items-center gap-2">
                                                <span>📝 メモ・詳細</span>
                                                {match.going_first && <span className="bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded">{match.going_first}</span>}
                                            </div>
                                            <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                                                <button
                                                    onClick={() => startEditing(match)}
                                                    className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded hover:bg-blue-50 hover:text-blue-600 transition font-bold border border-gray-200 flex items-center gap-1"
                                                >
                                        ...
                                        <div className="pl-3 border-l-4 border-blue-100 pt-1 pb-2" onClick={(e) => e.stopPropagation()}>
                                                ...
                                                <div className="space-y-4 p-2 bg-gray-50 rounded-lg border border-gray-200 shadow-inner">
                                                                ...
                                                                {(['先攻', '後攻'] as const).map((g) => (
                                                                    <button
                                                                        ...
                                                                        className={`flex-1 py-2.5 px-1 rounded-xl text-xs font-bold transition-all shadow-sm ${editForm.going_first === g
                                                                            ? 'bg-blue-700 text-white translate-y-[-1px] shadow-blue-100 ring-2 ring-blue-400'
                                                                            : 'bg-white text-gray-400 border-2 border-gray-100 hover:bg-gray-50 hover:border-blue-200'
                                                                            }`}
                                                                    >
                                                                        {g}
                                                                    </button>
                                                        ...
                                                        <div className="grid grid-cols-2 gap-3">
                                                            <div className="space-y-1">
                                                                ...
                                                                <select
                                                                    ...
                                                                    className="w-full px-2 py-2 bg-white border-2 border-gray-200 rounded-lg text-xs font-bold text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400 transition cursor-pointer appearance-none"
                                                    ...
                                                    <div className="flex justify-end gap-2 pt-2 border-t border-gray-200">
                                                        ...
                                                        <button
                                                            ...
                                                            className="px-2.5 py-2 text-sm font-bold bg-blue-500 text-white rounded-md hover:bg-blue-600 shadow-sm transition disabled:opacity-50"
                                                            disabled={updating}
                                                        >
                                                            {updating ? '保存中...' : '保存'}
                                                        </button>
