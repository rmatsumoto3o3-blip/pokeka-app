...
export default function PracticeTool({
    ...
    const [deckCode1, setDeckCode1] = useState(initialCode1 || searchParams.get('code1') || '')
    const [deckCode2, setDeckCode2] = useState(initialCode2 || searchParams.get('code2') || '')
    ...
    // Auto-load if deck codes are in URL (page mode)
    useEffect(() => {
        if (embedded) return
    ...
    return (
        <div
          className={embedded
            ? "w-full h-full overflow-y-auto flex flex-col transition-colors duration-700"
            : "h-[100dvh] md:h-auto md:min-h-screen p-1 sm:p-4 pb-[env(safe-area-inset-bottom)] overflow-y-auto md:overflow-auto flex flex-col transition-colors duration-700"
          }
          style={{
        ...
        >
            <div className={embedded ? "w-full relative z-10 p-1 sm:p-4" : "max-w-[1800px] mx-auto w-full relative z-10"}>
                ...
                <div className="mb-2 md:mb-4 flex justify-between items-center hidden md:flex">
                    ...
                    {onClose && (
                        <button
                            onClick={onClose}
                            className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg text-sm font-bold transition"
                        >
                            閉じる ×
                        </button>
                    )}
                ...
                {(!deck1.length || !deck2.length) && (
                    <div className="bg-white rounded-xl shadow-lg p-4 mb-4">
                        ...
                        <button
                            ...
                            className="mt-4 w-full px-6 py-3 bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition"
                        >
                ...
                {/* Content description for SEO/AdSense (Value) - only on the full page, not in modal */}
                {!embedded && (
                        ...
                        <div className="bg-white rounded-xl p-6 md:p-10 shadow-sm border border-gray-100">
                            ...
                            <div className="space-y-6 text-gray-600 leading-relaxed">
                                ...
                                <section className="bg-blue-50 p-5 rounded-xl border border-blue-100">
                                    <h3 className="font-bold text-blue-900 mb-2">💡 練習の質を上げるテクニック</h3>
                                    <ul className="list-disc list-inside space-y-2 ml-2 text-sm text-blue-950">
...
// Sub-components for D&D in Practice Tool
