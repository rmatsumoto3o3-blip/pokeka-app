...
export default function DeckDetailManager({
    ...
    const handleMoveToArchetype = async (targetArchId: string) => {
        ...
        try {
            ...
            // If we moved out of the current view context (e.g. we were viewing a folder and moved to another),
            // effectively this view is stale. 
            // If we were just viewing a deck and put it in a folder, we are now viewing that folder implicitly?
                                ...
                                {cardsLoading ? (
                                    ...
                                    <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-1.5">
                                        {deckCards.map((card, i) => (
                                            <div key={i} className="aspect-[2/3] bg-gray-200 rounded flex items-center justify-center relative group cursor-pointer hover:ring-2 ring-blue-400 overflow-hidden">
                                                {card.imageUrl ? (
                                    ...
                                    {currentVariant?.sideboard_cards && currentVariant.sideboard_cards.length > 0 ? (
                                        currentVariant.sideboard_cards.map((card, i) => (
                                            <div
                                                ...
                                                onClick={() => handleDeleteSideboardCard(i)} // Temp delete on click
                                            >
                                                {card.imageUrl ? (
                                                    <img src={card.imageUrl} alt={card.name} className="w-full h-full object-cover" />
                                                ) : (
                                                ...
                                                {/* Delete Overlay */}
                                                <div className="absolute inset-0 bg-red-500/50 hidden group-hover:flex items-center justify-center text-white font-bold text-xs">削除</div>
                                            </div>
                                        ))
                                    ) : (
