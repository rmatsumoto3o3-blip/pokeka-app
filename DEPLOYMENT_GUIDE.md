...
取得予定（または取得済み）のドメインをVercelに紐付けます。

### Vercel側の設定
1.  Vercelのプロジェクトページで **"Settings"** > **"Domains"** を開きます。
2.  取得したドメイン名（例: `pokemeta.com`）を入力し、**"Add"** をクリックします。
3.  **"Recommended"** (推奨設定) が表示されます。
    *   通常は `Aレコード` (76.76.21.21) または `CNAMEレコード` (cname.vercel-dns.com) の設定値が表示されます。この数値をメモしてください。

### お名前.comでの設定
1.  [お名前.com Navi](https://navi.onamae.com/) にログインします。
2.  **"DNS設定/転送設定"** > **"DNSレコード設定を利用する"** を選択します。
3.  対象のドメインを選択し、**"入力画面へ進む"** をクリックします。
4.  以下のレコードを追加します（Vercelの指示に従ってください）。
...
### 完了確認
VercelのDomains画面に戻り、設定したドメインに緑色のチェックマーク ✅ が付けば接続完了です（反映まで最大24〜48時間かかる場合があります）。
