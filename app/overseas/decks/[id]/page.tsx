...
import { getOverseasDeckAction } from '@/app/actions'
import { mockOverseasResults } from '@/lib/overseasData'

export const revalidate = 3600
export const dynamicParams = true

export function generateStaticParams() {
    return mockOverseasResults.map(result => ({ id: result.id }))
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
    const { id } = await params
    const response = await getOverseasDeckAction(id)
    if (!response.data) return { title: 'Deck Not Found | PokéLix' }
    return {
        title: `${response.data.archetype?.nameEn || 'Overseas Deck'} · ${response.data.result.rankLabel} | PokéLix`,
