...
// 内部リンク用：同じアーキタイプの他の入賞デッキ（Firebase environmentDecks/unionarena）。
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
    ...
    const jsonLd = {
        ...
        '@type': 'Article',
        headline: `${displayName}（${archetype}）| ユニアリ デッキレシピ`,
        ...
        description: `ユニアリ（ユニオンアリーナ）${archetype}デッキ「${displayName}」の大会結果とデッキレシピ。`,
        mainEntity: {
            '@type': 'ItemList',
            name: 'Deck List',
                ...
                {siblings.length > 0 && (
                    <section className="mt-8 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <ul className="divide-y divide-gray-100">
                            {siblings.map((d) => (
                                <li key={d.deckCode}>
                                    <a href={`/unionarena/decks/${encodeURIComponent(d.deckCode)}`} className="flex items-center justify-between py-2.5 hover:text-blue-600">
